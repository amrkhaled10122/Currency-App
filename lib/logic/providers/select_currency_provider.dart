...
class SelectCurrencyProvider with ChangeNotifier {
  ...
  List<CurrencyViewModel> currenciesList = <CurrencyViewModel>[];
  ...
  void selectCurrencyItem(int index, bool toConversion, CurrencyBloc currencyBloc, BuildContext context) {
    _makeCurrencyItemSelected(index);
    _refreshCurrentCurrencyValue(toConversion, context, currencyBloc, index);
    notifyListeners();
  }

  void _refreshCurrentCurrencyValue(
      bool toConversion, BuildContext context, CurrencyBloc currencyBloc, int index) async {
    if (toConversion) {
      await currencyBloc.refreshToConversionValueWithIndex(index);
    } else {
      await currencyBloc.refreshFromConversionValueWithIndex(index);
    }
