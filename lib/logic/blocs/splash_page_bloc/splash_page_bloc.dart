...
class SplashPageBloc extends Bloc<SplashPageEvent, SplashPageState> {
  ...
  SharedPreferencesService get _sharedPreferences => GetIt.I.get<SharedPreferencesService>();
  ...
  FutureOr<void> _onBlocLoad(SplashPageLoadEvent event, emit) async {
    ...
    try {
      ...
      if (_seen != null) {
        Timer((const Duration(seconds: 3)), () => _forceHomePage(event.context));
      } else {
        Timer((const Duration(seconds: 3)), () => _forceIntroPage(event.context));
      }
  ...
  _forceIntroPage(context) => _navigation.openIntroPage(context, withReplacement: true);

  _forceHomePage(context) => _navigation.openHomePage(context, withReplacement: true);
}
