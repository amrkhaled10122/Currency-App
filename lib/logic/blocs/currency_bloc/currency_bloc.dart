...
class CurrencyBloc extends Bloc<CurrencyEvent, CurrencyState> {
  ...
  String get formattedDate => DateFormat('MMMM dd, yyyy').format(selectedDateValue);
  ...
  final _fromConversionValueIndicatorController = BehaviorSubject<CurrencyViewModel>();
  final _toConversionValueIndicatorController = BehaviorSubject<CurrencyViewModel>();
  ...
  final _selectedDateController = BehaviorSubject<DateTime>.seeded(DateTime.now());
  ...
  Future<void> _checkIsCurrencyCached() async {
    if (_hiveService.checkDateIsCached(currentDate)) {
      final cachedCurrenciesData = _hiveService.fetchCachedCurrenciesWithDate(currentDate);
  ...
  Future<void> refreshFromConversionValueWithIndex(int value) async {
    ...
    final typedValue = typedConversionValue *
        await _serverRepository.getExchangeCurrency(base: fromConversionValue.code!, currency: toConversionValue.code!);
  ...
  void insertToTypedValueRate(int value, BuildContext context, AppLocalizations localization) async {
    if (typedConversionValue < maxValueLimit) {
      _typedConversionValueController.add((typedConversionValue * 10) + value);
      final typedValue = typedConversionValue * conversionValue;

      _conversionValueController.add(typedValue);
      autoRefreshConversionRate();
    } else {
      showExceedInputLimitInfoDialog(context: context, localization: localization);
    }
  }

  void deleteDigitOneByOneFromTypedValue() {
    if (typedConversionValue > 1) {
      _typedConversionValueController.add(typedConversionValue.makeLastDigitDroppped);
    } else {
  ...
  void autoRefreshConversionRate() async {
    if (typedConversionValue != 0) {
      _populateConversionsToHistory(
          typedConversionValue, conversionValue, fromConversionValue.code, toConversionValue.code, currentDate);
  ...
  void _addDefualtValuesToControllers() async {
...
extension CurrencyBlocFromConversionValueIndicatorControllerRxExtension on CurrencyBloc {
  Stream<CurrencyViewModel> get fromConversionStream => _fromConversionValueIndicatorController.stream;
  CurrencyViewModel get fromConversionValue => _fromConversionValueIndicatorController.value;
}

extension CurrencyBlocToConversionValueIndicatorControllerRxExtension on CurrencyBloc {
  Stream<CurrencyViewModel> get toConversionStream => _toConversionValueIndicatorController.stream;
  CurrencyViewModel get toConversionValue => _toConversionValueIndicatorController.value;
}

extension CurrencyBlocTypedConversionValueControllerRxExtension on CurrencyBloc {
  Stream<double> get typedConversionStream => _typedConversionValueController.stream;
